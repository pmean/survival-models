...
Likelihood: l(beta) = prod f^c S^(1-c)

Partial likelihood = prod exp(X beta)/ sum exp(X beta)

h(t) = (P[t <= T <= t+delta t]/delta t) / P[T>t]
lambda(t|X)=lamba0(t)exp(X beta)

S*t|X)=[S0(t)]^exp(X beta)

Define cumulative hazard H(t).

LR test: G=2{Lp(beta)-Lp(0)}

Wald test: beta/se(beta)

Score test: partial Lp/partial beta / sqrt(I(beta))
...
Next let's look at the age groups. Compare the results below to Table 4.6.
...
Talk about the interpretation of beta and how to calculate the effect of a five year change in age.

More analyses to do:

* Show how to get S0(t), H0(t), h0(t)

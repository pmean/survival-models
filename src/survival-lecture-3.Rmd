...
Start with the simplest case, the exponential distribution. The exponential distribution has a constant hazard rate, which we will let equal $\lambda$ for the first group and $2 \lambda$ for the second group. The larger the value of $\lambda$ the less time you need to accumulate 75 deaths total. 

Let's choose $\lambda=0.1$. Then with an average of three year's worth of follow up, the probability of seeing a death is $1-e^{-3*0.1}=$ `r round(1-exp(-0.3), 2)` in the first group and $1-e^{-3*0.2}=$ `r round(1-exp(-0.6), 2)` in the second group.

Note that the probability of death is not doubled when you double the hazard rate.

Divide the number of deaths by the probability of death to get the total number studied in each group.

$n_1 =$ 25 / `r round(1-exp(-0.3), 2)` 
= `r round(25 / (1-exp(-0.3)), 1)`.

$n_2 =$ 50 / `r round(1-exp(-0.6), 2)` 
= `r round(50 / (1-exp(-0.6)), 1)`.

There's one more thing to consider. Some patients might drop out during the study. You could assume, for example, that the drop out rate before the third year of the study is 2% and that dropouts are uniformly distributed throughout the three years. Divide each of the sample sizes by 0.98 to get

$n_1 =$ 25 / `r round(1-exp(-0.3), 2)` / 0.98 
= `r round(25 / (0.98*(1-exp(-0.3))), 1)`.

$n_2 =$ 50 / `r round(1-exp(-0.6), 2)` / 0.98 
= `r round(50 / (0.98*(1-exp(-0.6))), 1)`.
